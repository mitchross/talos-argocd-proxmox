...
base=sys.argv[1];size=512*1024**2;block=1024**2;limit=90
result={'startedAt':datetime.datetime.now(datetime.timezone.utc).isoformat(),'path':base,'sizeBytes':size,'method':'one Python process; QD1; 1 MiB O_DIRECT sequential write/read; 128 x 8 KiB buffered write+fsync; O_DIRECT read; unique scratch file'}
...
try:
 ...
 cap=pathlib.Path('/sys/fs/cgroup/memory.max');cur=pathlib.Path('/sys/fs/cgroup/memory.current')
 if cap.exists() and cap.read_text().strip()!='max':
  result['memoryHeadroomBytes']=int(cap.read_text())-int(cur.read_text())
  if result['memoryHeadroomBytes'] < 128*1024**2:raise RuntimeError('less than 128 MiB cgroup memory headroom; skipped')
 folder=tempfile.mkdtemp(prefix='.capacity-benchmark-20260920-',dir=base);file=folder+'/payload.bin'
 fd=os.open(file,os.O_RDWR|os.O_CREAT|os.O_EXCL|os.O_DIRECT,0o600);buf=mmap.mmap(-1,block);buf.write(os.urandom(block));buf.seek(0)
 ...
 os.close(fd);fd=os.open(file,os.O_RDWR);os.lseek(fd,0,os.SEEK_SET)
