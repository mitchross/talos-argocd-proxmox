...
| `infrastructure/storage/rustfs-lifecycle/` | GitOps-managed RustFS bucket lifecycle policy for abandoned database backup prefixes |
...
### RustFS lifecycle policy for abandoned database backup prefixes

CNPG/Barman retention controls active database backup lineages. The
`rustfs-lifecycle` controller handles the separate case: old `serverName`
prefixes that are no longer active after a database lineage cutover.

The controller lives at `infrastructure/storage/rustfs-lifecycle/` and is
generated by the infrastructure ApplicationSet. It owns the bucket lifecycle
configuration for `postgres-backups` through a PostSync Argo CD hook Job:

- `postgres-backups-lifecycle-cm.yaml` contains the full lifecycle JSON.
- `postgres-backups-lifecycle-job.yaml` runs `aws s3api put-bucket-lifecycle-configuration`.
- `argocd.argoproj.io/hook-delete-policy: BeforeHookCreation` lets the Job
  re-run after ConfigMap changes without hitting Kubernetes Job immutability.
- The PUT operation replaces the whole bucket lifecycle config, so keep every
  lifecycle rule in the one ConfigMap.

Current rule:

| Prefix | Reason | Expiration |
|--------|--------|------------|
| `cnpg/gitea/gitea-database-v1/` | Gitea moved from v1 to v2 lineage on 2026-05-02 after worker-node deletion left the v1 prefix as dead history | 60 days |

Use this pattern only for abandoned lineages. Do not add active CNPG
`serverName` prefixes here; active database retention belongs in the CNPG
backup configuration.

Verification:

```bash
kubectl get application rustfs-lifecycle -n argocd
kubectl get pods -n rustfs-lifecycle
kubectl logs -n rustfs-lifecycle job/postgres-backups-lifecycle-apply
kubectl get configmap postgres-backups-lifecycle -n rustfs-lifecycle -o yaml
```

Future abandoned lineages, such as an old `temporal-database-v1` prefix after a
verified v2 cutover, should be added by editing the lifecycle ConfigMap and
