...
- All four ApplicationSets use strict Go templates (`missingkey=error`). Git
  directory fields are objects in Go-template mode, so use
  `{{ .path.path }}` / `{{ .path.basename }}` rather than the deprecated
  fasttemplate forms. The render CI rejects any non-Component kustomization
  that produces zero Kubernetes objects.
