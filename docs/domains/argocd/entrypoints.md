...
| `appsets/` | Directory discovery for user apps plus explicit metadata discovery for infrastructure, databases, and monitoring |
...
| `appsets/infrastructure-appset.yaml` | ApplicationSet | 4 | Reads selected explicit metadata under `clusters/talos/infra/*/` | N/A |
| `appsets/database-appset.yaml` | ApplicationSet | 4 | Reads explicit metadata under `clusters/talos/database/*/*/`; uses `selfHeal: false` for DR | N/A |
| `appsets/monitoring-appset.yaml` | ApplicationSet | 5 | Reads explicit metadata under `clusters/talos/monitoring/*/` after core infra | N/A |
| `appsets/my-apps-appset.yaml` | ApplicationSet | 6 | Directory-discovers `clusters/talos/apps/*/*` after core storage and restore foundations | N/A |
...
- `project-nomad` is intentionally managed by `appsets/my-apps-appset.yaml` as a single bundled app at `clusters/talos/apps/home/project-nomad`. The directory generator matches exactly the category/app level, so its child folders are resources inside that app, not generated Argo CD Applications.
...
- Manifest-generation annotations use `.` for the Application source and absolute `/manifests/...` paths for shared bases. A source-relative `clusters/...` value is invalid.
...
Project Nomad is not special to Argo CD; it is special only in repo shape. The Talos apps ApplicationSet directory generator matches `clusters/talos/apps/*/*`, so `clusters/talos/apps/home/project-nomad` is the generated Application boundary.
...
Do not broaden the app generator or add separate child entrypoints under
Project Nomad unless those children should become separate Argo CD
Applications. If Project Nomad should ever split, change the discovery and
validation model deliberately at the same time.
