...
| `appsets/infrastructure-appset.yaml` | ApplicationSet | 4 | Reads selected `.argocd/config.json` files under `manifests/infra/*/deploy-targets/talos/` | N/A |
| `appsets/database-appset.yaml` | ApplicationSet | 4 | Discovers `manifests/database/*/*/deploy-targets/talos/.argocd/config.json`; uses `selfHeal: false` for DR | N/A |
| `appsets/monitoring-appset.yaml` | ApplicationSet | 5 | Discovers `manifests/monitoring/*/deploy-targets/talos/.argocd/config.json` after core infra | N/A |
| `appsets/my-apps-appset.yaml` | ApplicationSet | 6 | Discovers `manifests/apps/*/*/deploy-targets/talos/.argocd/config.json` after core storage and restore foundations | N/A |
...
- `project-nomad` is intentionally managed by `appsets/my-apps-appset.yaml` as a single bundled app at `manifests/apps/home/project-nomad/deploy-targets/talos`. Its child folders are resources inside that app, not generated Argo CD Applications.
- `rustfs-lifecycle` is not a standalone Argo entrypoint. It is generated by `appsets/infrastructure-appset.yaml` from `manifests/infra/rustfs-lifecycle/deploy-targets/talos` at wave 4.
- Global ignore rules for HTTPRoute, ExternalSecret, and PVC restore fields live in `manifests/infra/argocd/deploy-targets/talos/values.yaml`; AppSets should only carry app-specific ignore rules.
...
Project Nomad is not special to Argo CD; it is special only in repo shape. The Talos apps ApplicationSet discovers metadata files with `manifests/apps/*/*/deploy-targets/talos/.argocd/config.json`, so `manifests/apps/home/project-nomad/deploy-targets/talos` is the generated Application boundary.
...
ever split, add child deploy targets deliberately and update the
