...
2. **The Cluster CR** — declares "I want a Postgres cluster called X with these resources, this version, this backup config." Lives in `clusters/talos/database/cloudnative-pg/<app>/`.
...
```
clusters/talos/database/cloudnative-pg/temporal/
...
Copy an existing Talos DB entrypoint such as `clusters/talos/database/cloudnative-pg/gitea/` to the new database path, rename names + owner + image + initdb SQL. Set `base/cluster.yaml` `serverName` to `<newapp>-database-v1`. Set `overlays/recovery/bootstrap-patch.yaml` `externalClusters.serverName` to the same `-v1` (placeholder until first DR). Add `.argocd/config.json`, commit, and push. The Database AppSet auto-discovers `clusters/talos/database/*/*/.argocd/config.json` — no appset edits needed.
