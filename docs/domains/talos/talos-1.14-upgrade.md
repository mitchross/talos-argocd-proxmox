...
2026-06-11 and required a reprovision. The single-node GPU template *does*
already run UserVolume-backed Longhorn tiers
(`/var/mnt/longhorn-nvme1`, `/var/mnt/longhorn-ssd-flash`), but its
`talos-ephemeral` tier still lives on `/var/lib/longhorn`, so it needs the same
opt-out.
...
| `.machine.kernel.modules` | `KernelModuleConfig` | `patches/gpu-worker.yaml`, `patches/single-node-gpu.yaml` |
...
The matching `siderolabs/extensions` v1.14.0-beta.1 release carries NVIDIA
production **595.71.05** and container-toolkit **1.19.1** — the same driver
branch the GPU nodes run today, so no driver-generation change rides along with
this upgrade. The Dell GTX 1050 Ti worker stays on the 580.x LTS extensions for
Pascal support.
...
|---|---|---|
| **Cilium 1.19.6 on Kubernetes 1.37** | Cilium's published compatibility matrix tops out well below 1.37. The cluster already runs 1.19.6 on 1.36.3 successfully, so this is one step further off the tested grid, not the first. | `cilium status` degraded, agents crash-looping, or Hubble certificate errors right after the control-plane upgrade. |
| **Longhorn 1.12.0 on Kubernetes 1.37** | Same shape — the chart predates 1.37. | Longhorn manager pods failing to list/watch after the upgrade. |
...
**Expected:** empty, apart from the CNPG DR gates that are intentionally
`automated.enabled: false` (immich, paperless, temporal and their consumers).
...
- Version pins: `omni/cluster-template/cluster-template.yaml` and
  `cluster-template-singlenode-gpu.yaml`
