...
`redis-instance/redis-master-0` is **✅ backup-exempt as of 2026-06-01** (`chore(redis): mark redis data backup-exempt`). It is paperless-ngx's Celery broker + a redis-commander UI — a cache/broker, not a system of record (paperless durable data = its data/media PVCs + CNPG Postgres). It was marked `backup-exempt=true` with the FQ reason key, its inline argocd RS/RD were pruned, and its PVC `dataSourceRef` removed — which **cleared the long-standing `redis-instance` ComparisonError** (now `Synced/Healthy`). **There are no remaining migration candidates.** Everything is migrated (24/24 DR_COMPLETE), `backup-exempt` (posthog ×3 + redis), or a permanent CNPG exclusion.
...
| immich/library | derived files (~6GiB actual / 300Gi prov.) | 568 | **C — ✅ DONE + restore-validated 2026-05-31** | Migrated (`49b97920`, tier=daily `23 2 * * *`). Empty reset; first operator backup Successful. **DR-completion drill (`ecd6009e`/`e680e696`): added `dataSourceRef → library-dst`, then delete→recreate→VolSync populator restore validated byte-identical (sha256 match; sentinel cross-verified from both server & ML pods). NO double-recreate** (stale-render mitigation held). New uid after drill→`5e5a26cc`. Quiesced BOTH `immich-server` + `immich-machine-learning` (ML co-mounts library RWO); `init-library-markers` initContainer recreated all 6 `.immich` markers; server came up 1/1. Originals on exempt NFS `nfs-photos` (untouched); DB is CNPG (intact → broken/missing-asset UI, accepted — restore proves CURRENT working-set recreate, does NOT fix prior DB↔asset mismatch). Scale-back-up used the hard-refresh-to-clear-then-sync pattern (ArgoCD stale-cache). |
| redis-instance/redis-master-0 | Redis AOF + RediSearch/ReJSON | n/a | **A — ✅ EXEMPT 2026-06-01** | paperless Celery broker + redis-commander UI = cache/broker, not a system of record. `backup-exempt=true` + FQ reason; inline RS/RD pruned; PVC dsr removed → cleared the redis-instance ComparisonError. Never migrate; no pvc-plumber labels. PVC stays Bound (data not deleted; recreate-empty by design). |
...
~~n8n (canary)~~ ✅ DONE → ~~home-assistant~~ ✅ DONE → ~~gitea~~ ✅ DONE → ~~paperless data~~ ✅ DONE → ~~paperless media~~ ✅ DONE → ~~immich~~ ✅ DONE (`49b97920`, empty reset) → ~~redis-instance/redis-master-0~~ ✅ **EXEMPT 2026-06-01** (paperless Celery broker, disposable; backup-exempt, inline RS/RD pruned, dsr removed → ComparisonError cleared). ~~posthog~~ ✅ **EXEMPT 2026-05-31** (disposable; backup-exempt, RS/RD removed, never migrate — `969a8e35`). **CNPG: never.** **Migration campaign CLOSED — every PVC is migrated (24/24 DR_COMPLETE), backup-exempt (posthog ×3 + redis), or a permanent CNPG exclusion. No remaining candidates.**
