...
- **KubePrism port (`7445`) is unchanged** across 1.12 → 1.13 → 1.14.
  ...
  field working.
- **Cilium 1.19.x** is the pairing this repo runs
  (`infrastructure/networking/cilium/kustomization.yaml` pins 1.19.6).
  ...
  **Caveat on 1.14:** it ships Kubernetes 1.37, which is beyond Cilium's
  published compatibility matrix. Run `cilium status` right after any
  control-plane roll rather than assuming.
