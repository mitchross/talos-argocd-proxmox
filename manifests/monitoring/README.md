...
```mermaid
graph TB
    subgraph "Collection (manifests/infra/opentelemetry-operator/)"
    ...
    subgraph "Local Storage (manifests/monitoring/)"
...
|-----------|----------|---------|
| **OTEL Operator** | `manifests/infra/opentelemetry-operator/` | Manages Collectors + auto-instrumentation |
...
| **Prometheus** | `manifests/monitoring/prometheus-stack/` | Metrics storage, alerting, Grafana |
| **Loki** | `manifests/monitoring/loki-stack/` | Log storage (S3 on RustFS) |
| **Tempo** | `manifests/monitoring/tempo/` | Trace storage (S3 on RustFS) |
...
3. **Instrumentation CR not matching.** The `Instrumentation` CR
   (`manifests/infra/opentelemetry-operator/instrumentation.yaml`)
...
| **Installed via** | `manifests/monitoring/prometheus-stack/` (Wave 5) | `manifests/infra/metrics-server/` (Wave 4) |
...
> the pod silently runs with no creds. See `manifests/monitoring/CLAUDE.md`.
...
- GPU alerts/dashboard: `manifests/monitoring/prometheus-stack/gpu-alerts.yaml`, `gpu-dashboard.yaml`
- OTEL Collectors: `manifests/infra/opentelemetry-operator/collector-*.yaml`
...
### Common pitfalls (see `manifests/monitoring/CLAUDE.md` for details)
