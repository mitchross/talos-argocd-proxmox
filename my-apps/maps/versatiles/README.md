...
- `https://maps.vanillax.me/styles/light.json` and `…/styles/dark.json` —
  **pre-generated static MapLibre styles** we host for external clients that
  ...
  dark = `eclipse`, generated by `styles/generate.mjs` and served by the
...
map.setStyle(colorful({ baseUrl: 'https://maps.vanillax.me' }));  // dark: eclipse(...)
```
...
curl -s https://maps.vanillax.me/styles/dark.json | head -c 60   # {"version":8,"name":"versatiles-eclipse"...
```
